...
router.get('/', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const configs = await configService_1.ConfigService.getAllConfigs();
        res.json({ configs });
    }
    catch (error) {
...
router.get('/active', auth_1.authenticateToken, async (req, res) => {
    try {
        const config = await configService_1.ConfigService.getActiveConfig();
        if (!config) {
...
router.get('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const config = await configService_1.ConfigService.getConfigById(id);
        if (!config) {
...
router.put('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const config = await configService_1.ConfigService.updateConfig(id, updates);
        res.json({ config });
    }
    catch (error) {
...
router.put('/:id/activate', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const config = await configService_1.ConfigService.setActiveConfig(id);
        res.json({ config });
    }
    catch (error) {
...
router.delete('/:id', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        await configService_1.ConfigService.deleteConfig(id);
        res.status(204).send();
    }
    catch (error) {
...
router.post('/test-config', auth_1.authenticateToken, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const response = await configService_1.ConfigService.testConfiguration(testConfig);
        res.json({
            success: true,
            response,
